...
struct QuickChatTextContext: Equatable, Sendable {
    ...
    var label: String {
        "\(self.appName) — \(self.windowTitle) (\(self.characterCount) chars)"
    }
}
...
enum QuickChatFocusedTextCaptureService {
    ...
    static func capture() async -> QuickChatTextContextCaptureOutcome {
        ...
        guard application.processIdentifier != getpid(),
            ...
            return .failed("Focus another app before attaching its text.")
        }
        ...
        guard hasPermission else {
            ...
            guard result[.accessibility] == true else {
                return .failed("Accessibility access is required to attach text from \(appName).")
            }
        ...
        appName: String) async -> QuickChatTextContextCaptureOutcome
    {
        ...
        guard focusedWindowError == .success,
            ...
            return .failed("No focused window is available in \(appName).")
        }
        ...
        guard snapshot.1.textEntryCount > 0 else {
            return .failed("No readable text was found in \(appName).")
        }
    ...
    private static func confirmAccessibilityRequest(appName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = "Allow OpenClaw to read text from \(appName)"
        alert.informativeText = "Attaching focused-window text uses macOS Accessibility access."
